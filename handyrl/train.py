...
from .connection import MultiProcessJobExecutor, accept_socket_connections
from .environment import make_env, prepare_env
from .losses import compute_target
from .model import ModelWrapper, RandomModel, to_gpu, to_torch
from .util import bimap_r, map_r, rotate, trimap_r
...
def make_batch(episodes, args):
    ...
    for ep in episodes:
        ...
        v = np.array(
            [[replace_none(m["value"][player], [0]) for player in players] for m in moments], dtype=np.float32
        ).reshape(len(moments), len(players), -1)
        rew = np.array(
            [[replace_none(m["reward"][player], [0]) for player in players] for m in moments], dtype=np.float32
        ).reshape(len(moments), len(players), -1)
        ret = np.array(
            [[replace_none(m["return"][player], [0]) for player in players] for m in moments], dtype=np.float32
        ).reshape(len(moments), len(players), -1)
...
def forward_prediction(model, hidden, batch, args):
    ...
    if hidden is None:
        ...
        for t in range(batch["turn_mask"].size(1)):
            ...
            next_hidden = bimap_r(
                next_hidden, hidden, lambda nh, h: nh.view(h.size(0), -1, *h.size()[2:])
            )  # (..., B, P or 1, ...)
...
def compose_losses(outputs, log_selected_policies, total_advantages, targets, batch, args):
    ...
    entropy_loss = (
        entropy.mul(1 - batch["progress"] * (1 - args["entropy_regularization_decay"])).sum() * -args["entropy_regularization"]
    )
...
def compute_loss(batch, model, hidden, args):
    ...
    if "value" in outputs_nograd:
        ...
        if args["turn_based_training"] and values_nograd.size(2) == 2:  # two player zerosum game
            ...
            values_nograd = (values_nograd + values_nograd_opponent) / (
                batch["observation_mask"].sum(dim=2, keepdim=True) + 1e-8
            )
    ...
    return_args = (
        outputs_nograd.get("return", None),
        batch["return"],
        batch["reward"],
        args["lambda"],
        args["gamma"],
